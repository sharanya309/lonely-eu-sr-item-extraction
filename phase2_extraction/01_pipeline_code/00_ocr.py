...
def _skip_ids():
    # papers we must NOT re-attempt: already OCR'd (ok) or terminally bad (not_pdf/error)
    return _statuses("ok", "not_pdf", "error")
...
def submit(src, n):
    ...
    if badlog:
        ...
        print(f"  skipped {len(badlog)} non-PDF file(s) -> logged to {BAD_CSV.name}")
    if n:
        valid = valid[:n]
    if not valid:
