#!/usr/bin/env python3
"""
run_append.py — Unattended: process every REMAINING pdf and APPEND the new papers
to the existing data/final/master_flat.csv WITHOUT touching a single existing row.

What it does, in one command:
    1. 00_ocr.py auto        OCR all remaining PDFs (skips done + logs bad)   [existing]
    2. 01_extract.py auto    both coders extract the new papers                [existing]
...
def stages_00_to_03(poll=60):
    run("00_ocr.py", "auto")            # all remaining PDFs
    run("01_extract.py", "auto")        # both coders
